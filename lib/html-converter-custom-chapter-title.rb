...
module Asciidoctor
class CustomChapterTitleHtml5Converter < (Asciidoctor::Converter.for 'html5')
  register_for 'html5'

  register_for 'custom_chapter_title_html5'

  def convert_section node
    doc_attrs = node.document.attributes
    level = node.level
    # 索引の章は生成しない
    return if node.sectname == 'index'

    if node.caption
      title = node.captioned_title
    elsif node.numbered && level <= (doc_attrs['sectnumlevels'] || 3).to_i
      if level < 2 && node.document.doctype == 'book'
        # warn node.sectnum.length
        case node.sectname
        when 'chapter'
