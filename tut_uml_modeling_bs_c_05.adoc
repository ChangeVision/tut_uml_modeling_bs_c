...
作成したプログラムの全体を <<c_all_code01>> 、 <<c_all_code02>> に示します。
...
----
include::{sourcesdir}/score/bowling.h[]
----
...
[source,c,linenums,start=1]
----
include::{sourcesdir}/score/bowling.c[]
----


[[c_all_makefile]]
.`Makefile`
[source,makefile,linenums,start=1]
----
include::{sourcesdir}/score/Makefile[]
----
...
作成したプログラムの末尾に、テストするためのプログラムを追加します（ <<c_all_code03>> ）。
...
[source,c,linenums,start=1]
----
include::{sourcesdir}/score/main.c[]
----
<1> 新しいスコアシートを作成する。
<2> 作成したスコアシートにゲームを追加する。
<3> 追加したゲームを選ぶ（ここでは最後のゲームを選ぶ方法をとった）。
<4> ゲームへプレーヤーがエントリーする。このプレーヤーのこのゲーム用のスコアが用意される。
<5> スコアシートの作成日付とIDの表示。
<6> テスト用データ。1行が1人の1ゲーム分のピン数のデータ。
<7> 各プレイヤーの現在の投球インデックス。
<8> ゲームの終了まで繰り返し。
...
コマンドプロンプトを起動して（MacやLinuxならターミナルを起動して）、プログラムをビルドします（ <<c_all_code04>> ）。

[[c_all_code04]]
.`bowling` プログラムをビルドする
[source,console]
----
C:\Users\（演習用ディレクトリ）>cd score
C:\Users\（演習用ディレクトリ）>\score> make
----

ビルドできたら、実行しましょう（ <<c_all_code05>> ）。
...
[source,console]
----
C:\Users\（演習用ディレクトリ）>\score> ./bowling
...
.実行の様子を収めたビデオ（Ruby版のもの。 `videos` フォルダにあります）
